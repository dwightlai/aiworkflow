...
describe('identity api', () => {
  ...
  it('posts user lifecycle and integration app actions', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('/api/auth/admin/users', expect.objectContaining({
      method: 'POST',
      body: expect.stringContaining('"username":"new-user"')
    }));
    ...
    expect(fetchMock).toHaveBeenCalledWith('/api/auth/admin/users/user_new', expect.objectContaining({
      method: 'PUT',
      body: expect.stringContaining('"displayName":"New User Updated"')
    }));
  ...
  it('posts organization resource create and update actions', async () => {
    const fetchMock = mockFetch({
    ...
    await updateRole('role_ops', { organizationId: 'org_ops', name: 'Ops Role Updated', roleType: 'BUSINESS', externalRoleId: 'R-2', status: 'DISABLED' });
