...
import { Alert, Button, Card, Drawer, Form, Input, InputNumber, Modal, Select, Space, Statistic, Table, Tabs, Tag, Typography, message } from 'antd';
...
export function IdentityOrganizationPage({ defaultTab = 'organizations' }: IdentityOrganizationPageProps) {
  ...
  const saveUserMutation = useMutation({
    mutationFn: (values: UserFormValues) => {
      ...
      return userDrawer?.mode === 'edit' && userDrawer.record
        ? updateUser(userDrawer.record.id, payload)
        : createUser(payload);
    },
    onSuccess: async () => {
  ...
  const openCreateUser = () => {
    ...
    userForm.setFieldsValue({ username: '', password: '', displayName: '', mobile: null, email: null, organizationIds: [], roleCodes: ['app_user'] });
  };

  const openEditUser = (record: IdentityUser) => {
    setUserDrawer({ mode: 'edit', record });
    userForm.setFieldsValue({ ...record, password: '', roleCodes: record.roleIds, organizationIds: record.organizationIds ?? record.unitIds ?? [] });
  };
    ...
    <section style={pageStyle}>
          ...
          items={[
            {
              ...
              children: (
                <>
                  ...
                  <OrganizationTable organizations={organizations} loading={organizationsQuery.isLoading} onEdit={openEditOrg} onToggleStatus={(org) => saveOrgMutation.mutate({ ...org, status: org.status === 'ACTIVE' ? 'DISABLED' : 'ACTIVE' })} />
                </>
              )
            },
            {
              ...
              children: (
                <>
                  <Toolbar><Button type="primary" icon={<PlusOutlined />} onClick={openCreateUser}>新增用户</Button></Toolbar>
                  <UserTable users={users} loading={usersQuery.isLoading} onEdit={openEditUser} onStatus={(user, status) => updateUserStatusMutation.mutate({ userId: user.id, status })} onResetPassword={setPasswordUser} />
                </>
              )
            },
            {
              ...
              children: (
                <>
                  ...
                  <RoleTable roles={roles} loading={rolesQuery.isLoading} onEdit={openEditRole} onToggleStatus={(role) => saveRoleMutation.mutate({ ...role, status: role.status === 'ACTIVE' ? 'DISABLED' : 'ACTIVE' })} />
                </>
              )
            },
            {
              ...
              children: (
                <>
                  ...
                  <IntegrationAppTable apps={apps} loading={appsQuery.isLoading} onStatus={(app, status) => updateAppStatusMutation.mutate({ appId: app.id, status })} onSecret={(app) => createSecretMutation.mutate(app.id)} onScope={setScopeApp} />
                </>
              )
            }
          ]}
      ...
      <Drawer title={roleDrawer?.mode === 'edit' ? '编辑角色' : '新增角色'} open={Boolean(roleDrawer)} width={500} onClose={() => setRoleDrawer(null)} footer={<DrawerFooter onCancel={() => setRoleDrawer(null)} onSubmit={() => roleForm.submit()} loading={saveRoleMutation.isPending} />}>
        <Form form={roleForm} layout="vertical" onFinish={(values) => saveRoleMutation.mutate(values)}>
          {roleDrawer?.mode === 'create' ? <Form.Item name="code" label="角色编码" rules={[{ required: true, message: '请输入角色编码' }]}><Input /></Form.Item> : null}
...
function OrganizationTable({ organizations, loading, onEdit, onToggleStatus }: { organizations: Organization[]; loading: boolean; onEdit: (org: Organization) => void; onToggleStatus: (org: Organization) => void }) {
  const columns: ColumnsType<Organization> = [
    ...
    { title: '操作', width: 180, render: (_, org) => <Space><Button size="small" icon={<EditOutlined />} onClick={() => onEdit(org)}>编辑</Button><Button size="small" onClick={() => onToggleStatus(org)}>{org.status === 'ACTIVE' ? '停用' : '启用'}</Button></Space> }
  ];
...
function UserTable({ users, loading, onEdit, onStatus, onResetPassword }: { users: IdentityUser[]; loading: boolean; onEdit: (user: IdentityUser) => void; onStatus: (user: IdentityUser, status: string) => void; onResetPassword: (user: IdentityUser) => void }) {
  const columns: ColumnsType<IdentityUser> = [
    ...
    { title: '操作', width: 360, render: (_, user) => <Space wrap><Button size="small" icon={<EditOutlined />} onClick={() => onEdit(user)}>编辑</Button><Button size="small" icon={<UnlockOutlined />} onClick={() => onStatus(user, 'ACTIVE')}>启用</Button><Button size="small" onClick={() => onStatus(user, 'DISABLED')}>禁用</Button><Button size="small" icon={<LockOutlined />} onClick={() => onStatus(user, 'LOCKED')}>锁定</Button><Button size="small" icon={<KeyOutlined />} onClick={() => onResetPassword(user)}>重置密码</Button></Space> }
  ];
  return <Table rowKey="id" columns={columns} dataSource={users} loading={loading} pagination={false} size="middle" />;
}

function RoleTable({ roles, loading, onEdit, onToggleStatus }: { roles: Role[]; loading: boolean; onEdit: (role: Role) => void; onToggleStatus: (role: Role) => void }) {
  const columns: ColumnsType<Role> = [
    ...
    { title: '状态', dataIndex: 'status', render: statusTag },
    { title: '操作', width: 180, render: (_, role) => <Space><Button size="small" icon={<EditOutlined />} onClick={() => onEdit(role)}>编辑</Button><Button size="small" onClick={() => onToggleStatus(role)}>{role.status === 'ACTIVE' ? '停用' : '启用'}</Button></Space> }
  ];
  return <Table rowKey="id" columns={columns} dataSource={roles} loading={loading} pagination={false} size="middle" />;
}

function IntegrationAppTable({ apps, loading, onStatus, onSecret, onScope }: { apps: IntegrationApp[]; loading: boolean; onStatus: (app: IntegrationApp, status: string) => void; onSecret: (app: IntegrationApp) => void; onScope: (app: IntegrationApp) => void }) {
  const columns: ColumnsType<IntegrationApp> = [
    ...
    { title: '状态', dataIndex: 'status', render: statusTag },
    { title: '操作', width: 330, render: (_, app) => <Space wrap><Button size="small" onClick={() => onStatus(app, app.status === 'ACTIVE' ? 'DISABLED' : 'ACTIVE')}>{app.status === 'ACTIVE' ? '停用' : '启用'}</Button><Button size="small" icon={<KeyOutlined />} onClick={() => onSecret(app)}>生成 Key</Button><Button size="small" icon={<SafetyCertificateOutlined />} onClick={() => onScope(app)}>添加范围</Button></Space> }
  ];
...
function buildOrganizationTree(organizations: Organization[]): Organization[] {
  ...
  byId.forEach((org) => { if (org.children?.length === 0) delete org.children; });
  return roots;
}
...
function normalizeUser(values: UserFormValues): SaveUserRequest {
  return { username: values.username, password: values.password, displayName: values.displayName, mobile: emptyToNull(values.mobile), email: emptyToNull(values.email), organizationIds: values.organizationIds ?? [], roleCodes: values.roleCodes ?? [] };
}
