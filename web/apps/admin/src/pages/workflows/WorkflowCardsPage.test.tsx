...
const workflowApiMock = vi.hoisted(() => ({
  ...
  restoreWorkflow: vi.fn(async () => ({
    ...
    status: 'PUBLISHED',
    latestVersion: { version: 3 },
    updatedAt: '2026-05-28T10:23:00Z'
  })),
...
describe('WorkflowCardsPage', () => {
  ...
  it('restores an archived workflow from the card actions', async () => {
    workflowApiMock.listWorkflows.mockResolvedValueOnce({
      items: [
        {
          id: 'workflow-1',
          name: '客服意图识别',
    ...
    await waitFor(() => {
      expect(workflowApiMock.restoreWorkflow).toHaveBeenCalledWith('workflow-1');
    });
  });
