# AI Workflow Platform — 技术手册

> **版本**: v1.0  
> **最后更新**: 2026-06-04  
> **维护团队**: AI Workflow 研发组
...
- 10 种内置节点类型：开始、结束、LLM 调用、提示词模板、知识检索、HTTP 工具、条件判断、文本转换、内容模板、循环
- 基于索引的版本管理，支持草稿/发布/归档状态流转
...
**项目包结构** (`server/src/main/java/com/aiworkflow/`)：
...
├── common/                         # 通用模块
│   ├── api/
│   │   ├── ApiResponse.java        # 统一响应封装 {success, data, error}
│   │   └── ErrorResponse.java      # 错误信息 {code, message, requestId, details}
│   └── exception/
│       └── GlobalExceptionHandler.java  # 全局异常处理 (@RestControllerAdvice)
├── config/                         # 配置模块
│   ├── OpenApiConfig.java          # SpringDoc / Swagger 配置
│   ├── SecurityConfig.java         # Spring Security (CSRF 关闭, 全部放行)
│   └── StoreConfig.java            # Store Bean 条件装配
├── persistence/
│   └── JsonSupport.java            # JSON 序列化工具 (Jackson)
├── bot/                            # 智能体模块
│   ├── api/BotController.java
│   ├── domain/AiBot.java, BotSession.java, BotMessage.java, ...
│   ├── service/BotService.java, BotStore.java, InMemoryBotStore, MybatisBotStore
│   └── persistence/AiBotEntity.java, AiBotMapper.java, ...
├── integration/                    # 外部集成模块
│   ├── api/IntegrationAppController.java
│   ├── auth/AuthenticationClient.java (Feign)
│   └── organization/OrganizationClient.java (Feign)
├── knowledge/                      # 知识库模块
│   ├── api/KnowledgeBaseController.java, VectorStoreConfigController.java
│   ├── domain/KnowledgeBase.java, KnowledgeDocument.java, ...
│   ├── service/KnowledgeBaseService.java, DocumentTextExtractor.java, ...
│   └── persistence/KnowledgeBaseEntity.java, ...
├── model/                          # 模型管理模块
│   ├── api/ModelProviderController.java
│   ├── domain/ModelProvider.java
│   ├── service/ModelProviderService.java, ChatModelClient.java, EmbeddingClient.java
│   └── persistence/ModelProviderEntity.java
├── prompt/                         # 提示词模板模块
│   ├── api/PromptTemplateController.java
│   ├── domain/PromptTemplate.java
│   ├── service/PromptTemplateService.java
│   └── persistence/PromptTemplateEntity.java
└── workflow/                       # 工作流引擎模块
    ├── api/                         # REST 控制器 + DTO
    ├── domain/                      # Workflow, WorkflowNode, WorkflowEdge, ...
    ├── engine/                      # 执行引擎: 10 种 NodeExecutor
    ├── service/                     # WorkflowApplicationService, DagValidator
...
平台使用 Flyway 进行版本化的数据库 Schema 管理，共 16 张核心表（`agi_` 统一前缀）。
...
| V13 | 文档元数据扩展：dataset_type, processing_status, tags, category, source, raw_content 等 | — |
...
- **CSRF**：已关闭（API 服务，非浏览器表单提交场景）
- **认证授权**：所有请求默认放行（`SecurityConfig` 中 `anyRequest().permitAll()`）
- **认证委托**：认证功能由外部 `auth-service` 提供，平台通过 Feign 客户端调用
...
请求 → Spring Security Filter Chain
         ...
         └── Authorization Filter (permit-all)
              → Controller
                   → (可选) Feign → auth-service (Token 校验)
```

> **说明**：生产部署时，建议在前置网关层（如 Nginx/Spring Cloud Gateway）统一处理认证，或在 `SecurityConfig` 中配置具体的权限规则。
...
平台内置 10 种节点类型，每种节点有独立的执行器和配置面板：
...
```java
// com.aiworkflow.yourmodule.service.YourStore.java
...
```java
// com.aiworkflow.yourmodule.service.InMemoryYourStore.java
...
```java
// com.aiworkflow.yourmodule.persistence.YourEntity.java
...
// com.aiworkflow.yourmodule.persistence.YourMapper.java
@Mapper
public interface YourMapper extends BaseMapper<YourEntity> {}

// com.aiworkflow.yourmodule.service.MybatisYourStore.java
...
```java
// com.aiworkflow.workflow.engine.EmailSenderNodeExecutor.java
@Component
...
```java
// com.aiworkflow.knowledge.service.OpenAiEmbeddingClient.java
@Component
...
```java
// com.aiworkflow.integration.weather.WeatherClient.java
...
共 **9 个 Controller**，**45+ 个 API 端点**：

| Controller | 基础路径 | 端点数 | 说明 |
|------------|---------|--------|------|
| KnowledgeBase | `/api/knowledge-bases` | 22 | 知识库完整管理 |
| VectorStoreConfig | `/api/vector-store-configs` | 4 | 向量存储配置 |
| Workflow | `/api/workflows` | 6 | 工作流管理 |
| WorkflowRun | `/api` | 3 | 工作流运行 |
| OpenWorkflowRun | `/openapi/v1` | 2 | 开放接口 |
| ModelProvider | `/api/model-providers` | 4 | 模型管理 |
| PromptTemplate | `/api/prompts` | 4 | 提示词模板 |
| Bot | `/api/bots` | 8 | 智能体管理 |
| IntegrationApp | `/api/integration-apps` | 1 | 集成应用 |

> **完整的接口文档（含所有 Parameters、Request Body、Responses）请参考 [API.md](./API.md)**
